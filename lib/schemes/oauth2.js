...
export default class Oauth2Scheme {
  ...
  async _handleCallback (uri) {
    ...
    const callback = this.$auth.options.redirect.callback.replace('/admin', '')
    if (this.$auth.options.redirect && normalizePath(this.$auth.ctx.route.path) !== normalizePath(callback)) {
      return
    }
